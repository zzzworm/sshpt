...
class OutputThread(GenericThread):
    ...
    def writeOut(self, queueObj):
        ...
        if queueObj['port'] != 22:
             csv_out = "\"%s:%d\",\"%s\",\"%s\",\"%s\",\"%s\"" % (queueObj['host'], queueObj['port'], queueObj['connection_result'], datetime.datetime.now(), queueObj['commands'], queueObj['command_output'])
	else:
    ...
    while len(hostlist) != 0: # Only add items to the ssh_connect_queue if there are available threads to take them.
        for host in hostlist:
            if ssh_connect_queue.qsize() <= max_threads:
                ...
                rawhost = host
                if com_pos != -1:
		    port = int(host[com_pos+1:])
		    rawhost = host[:com_pos]	
                queueSSHConnection(ssh_connect_queue, rawhost, username, password, timeout, commands, local_filepath, remote_filepath, execute, remove, sudo, run_as, port)
...
def main():
    ...
    parser.add_option("-u", "--username", dest="username", default=os.environ['USERNAME'], help="The username to be used when connecting.  Defaults to the currently logged-in user.", metavar="<username>")
